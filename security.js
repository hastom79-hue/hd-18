(()=>{
  ...
  style.textContent=`
    ...
    .edition .flag-stage .flag-svg{display:block!important;width:142px!important;height:94px!important;border-radius:8px!important;box-shadow:0 4px 12px rgba(16,30,46,.18)!important;background:#fff}
    .edition[data-lang="en"] .flag-stage{gap:10px}
    .edition[data-lang="en"] .flag-stage .flag-svg{width:78px!important;height:52px!important;border-radius:7px!important}
  ...
  const FLAGS={
    Korea:`<svg class="flag-svg" viewBox="0 0 72 48" aria-label="Korea" role="img"><rect width="72" height="48" rx="2" fill="#fff"/><g transform="translate(36 24)"><path d="M0-10a10 10 0 1 1 0 20a5 5 0 1 0 0-10a5 5 0 1 1 0-10z" fill="#cd2e3a"/><path d="M0 10a10 10 0 1 1 0-20a5 5 0 1 0 0 10a5 5 0 1 1 0 10z" fill="#0047a0"/></g><g fill="#111"><g transform="translate(13 10) rotate(-33)"><rect x="-8" y="-5" width="16" height="2.5"/><rect x="-8" y="-.8" width="16" height="2.5"/><rect x="-8" y="3.4" width="16" height="2.5"/></g><g transform="translate(59 38) rotate(-33)"><path d="M-8-5h6v2.5h-6zm10 0h6v2.5H2zM-8-.8h6v2.5h-6zm10 0h6v2.5H2zM-8 3.4h6v2.5h-6zm10 0h6v2.5H2z"/></g><g transform="translate(59 10) rotate(33)"><path d="M-8-5h6v2.5h-6zm10 0h6v2.5H2z"/><rect x="-8" y="-.8" width="16" height="2.5"/><path d="M-8 3.4h6v2.5h-6zm10 0h6v2.5H2z"/></g><g transform="translate(13 38) rotate(33)"><rect x="-8" y="-5" width="16" height="2.5"/><path d="M-8-.8h6v2.5h-6zm10 0h6v2.5H2z"/><rect x="-8" y="3.4" width="16" height="2.5"/></g></g></svg>`,
  ...
  function fixFlags(){
    document.querySelectorAll('svg.flag-svg[aria-label]').forEach(el=>{
      ...
      if(!FLAGS[label]||el.dataset.hd18Accurate==='1')return;
      const wrap=document.createElement('div');wrap.innerHTML=FLAGS[label].trim();
      const replacement=wrap.firstElementChild;replacement.dataset.hd18Accurate='1';el.replaceWith(replacement);
    });
  }

  function enlargeWelcomeFlags(){
    document.querySelectorAll('.edition[data-lang]').forEach(card=>{
      if(card.dataset.hd18FlagLayout==='1')return;
      const flags=card.querySelector('.cardtop .flags');
      const body=card.querySelector('.cardbody');
      if(!flags||!body)return;
      const stage=document.createElement('div');stage.className='flag-stage';stage.innerHTML=flags.innerHTML;
      body.insertBefore(stage,body.firstChild);
      card.dataset.hd18FlagLayout='1';
      fixFlags();
    });
  }
  ...
  const boot=()=>{fixFlags();enlargeWelcomeFlags();const mo=new MutationObserver(()=>{fixFlags();enlargeWelcomeFlags()});mo.observe(document.documentElement,{subtree:true,childList:true});};
